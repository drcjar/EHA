( function() {
  ...
  var that = EHA.map.geojson = {
    ...
    init: function( sett, callback ) {
      ...
      var lyrOpts = {
        ...
        jsonlyr = ( this.jsonobj.level === 'counties' ) ? new L.TopoJSON( jsonSett.obj, lyrOpts ) :
        new L.geoJson( jsonSett.obj, lyrOpts );
    ...
    evHandl: function( feature, layer ) {
      ...
      clickme = function( e ) {
        if ( level === 'counties' ) {
          ...
          that.cnty.name = ( layer.feature.properties.county ).replace( /\s/g, '' );
          that.cnty.gid = layer.feature.properties.gid;
          counties = that.jsonlyr;
          that.disableSelection();
          that.getGeojson();
          return;
        }

        var w_name = layer.feature.properties.w.replace( /^\s\s*/, '' ).replace( /\s\s*$/, '' ),
          d_name = layer.feature.properties.d.replace( /^\s\s*/, '' ).replace( /\s\s*$/, '' );
        $( '.hoverBlock' ).attr( 'class', 'hoverBlock wardsClicked' );
        $( ".legendrr div.highl" ).removeClass( 'highl' );

        that.slctWardByName( w_name, d_name );
    ...
    disableSelection: function( flag ) {
      that.selection = EHA.wardSelected = false;
      that.slctdId = null;
      that.clear();
      $( '#clear' ).hide();
    ...
    getGeojson: function( c ) {
      ...
      if ( that.cnty.name != "" ) {
        ...
        loadJS( gender.charAt( 0 ) + "/" + getField() + "/" + that.cnty.gid + ".json", that.cnty, 'wards', c );
        if ( this.nrequests++ === 0 ) {
    ...
    startMe: function( flag ) {
      loadJS( '../counties/countiesTopo.json', 'counties', 'counties', flag );
    },
  ...
  var setSessionSelection = function() {
    ...
    that.currentBounds = EHA.map.lmap.getBounds();
    var center = EHA.map.lmap.getCenter(),
      zoom = EHA.map.lmap.getZoom(),
      gender = EHA.current.gender,
      options = {
        url: "backend/setRegisterSelection.php",
        data: "wardname=" + that.w_name + "&countyname=" + that.d_name + "&countyid=" + that.cnty.gid +
          "&wardId=" + that.slctdId + "&center=" + center.lat + "," + center.lng + "&zoom=" + zoom + "&gender=" + gender,
        type: "POST",
        success: function( data, textStatus, jqXHR ) {
  ...
  var callback = function( oldlyr ) {
    //var i = (that.jsonobj.level === 'counties' )? 1 : 2;
