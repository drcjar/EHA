...
( function( $ ) {
  function Plot( placeholder, data_, options_, plugins ) {
    ...
    function measureTickLabels( axis ) {
      ...
      function makeDummyDiv( labels, width ) {
        return $( '<div style="position:absolute;top:-10000px;' + width + 'font-size:smaller">' +
            '<div class="' + axis.direction + 'Axis ' + axis.direction + axis.n + 'Axis">' + labels.join( "" ) + '</div></div>' )
    ...
    function insertAxisLabels() {
      ...
      for ( var j = 0; j < axes.length; ++j ) {
        ...
        for ( var i = 0; i < axis.ticks.length; ++i ) {
          ...
          var lbl = Math.round( tick.label * 100 ) / 100;
          html.push( '<div class="tickLabel" style="' + style.join( ';' ) + '">' + lbl.toFixed( 2 ) + '</div>' ); //FEDE toFixed(2)
        }
        html.push( '</div>' );
      }
    ...
    function drawPointHighlight( series, point ) {
      ...
      octx.lineWidth = pointRadius * 4; //ADDED *5 (FEDE)
      octx.strokeStyle = $.color.parse( "#3CB371" ).scale( 'a', 0.7 ).toString(); //CHANGED FROM 0.5 TO 0.7 (FEDE)
      var radius = 3 * pointRadius,
        x = axisx.p2c( x ),
        y = axisy.p2c( y );
