...
JustGage = function( config ) {
  ...
  // delta 
  var dx = ( canvasW - widgetW ) / 5;
  var dy = ( canvasH - widgetH ) / 1;

  // title 
  var titleFontSize = 0; // ((widgetH / 8) > 10) ? (widgetH / 10) : 10;
  var titleX = dx + widgetW / 2;
  var titleY = 0; //dy + widgetH / 5;
  ...
  // label 
  var labelFontSize = 14 //((widgetH / 16) > 10) ? (widgetH / 16) : 10;
...
JustGage.prototype.refresh = function( val ) {
  ...
  this.canvas.getById( this.config.id + "-level" ).animate( {
    pki: [ val, this.config.min, this.config.max, this.params.widgetW, this.params.widgetH, this.params.dx, this.params.dy, this.config.gaugeWidthScale ],
    "fill": color
